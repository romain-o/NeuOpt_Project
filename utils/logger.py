...
def log_to_tb_train(tb_logger, agent, Reward, ratios, bl_val_detached, total_cost, total_cost_wo_feasible, grad_norms, 
    ...
    tb_logger.add_scalar('train/avg_cost', (total_cost).mean().item(), mini_step)
    tb_logger.add_scalar('train/avg_cost_wo_feasible', (total_cost_wo_feasible).mean().item(), mini_step)
    tb_logger.add_scalar('train/target_return', Reward.sum(-1).mean().item(), mini_step)
    ...
    tb_logger.add_scalar('train/c_cost_logger', c_cost_logger.item(), mini_step)
    ...
    if info is not None:
        tb_logger.add_scalar('info/if_if', info[0].mean(), mini_step)
        tb_logger.add_scalar('info/if_f', info[1].mean(), mini_step)
        tb_logger.add_scalar('info/f_if', info[2].mean(), mini_step)
        tb_logger.add_scalar('info/f_f', info[3].mean(), mini_step)
        tb_logger.add_scalar('info/if_if_2', info[4].mean(), mini_step)
        tb_logger.add_scalar('info/if_f_2', info[5].mean(), mini_step)
        tb_logger.add_scalar('info/f_if_2', info[6].mean(), mini_step)
        tb_logger.add_scalar('info/f_f_2', info[7].mean(), mini_step)

def log_to_tb_val(tb_logger, time_used, init_value, best_value, reward, 
                  costs_history, search_history, val_size, T, epoch):
    
    tb_logger.add_scalar('validation/avg_time',  time_used.mean() / val_size, epoch)
    ...
    for per in range(20,100,20):
        ...
        tb_logger.add_scalar(f'validation/avg_.{per}_cost', cost_.mean(), epoch)