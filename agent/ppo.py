...
class PPO:
    ...
    def load(self, load_path):
        ...
        # On charge le critique si disponible, mais ce n'est pas critique pour l'inférence
        if 'critic' in load_data:
            model_critic = get_inner_model(self.critic)
            model_critic.load_state_dict(load_data['critic'])
            
        # --- MODIFICATION : ON COMMENTE CES LIGNES ---
        # if load_data.get('rng_state') is not None:
        #     torch.set_rng_state(load_data['rng_state'])
        # if load_data.get('cuda_rng_state') is not None:
        #     torch.cuda.set_rng_state_all(load_data['cuda_rng_state'])
        # ---------------------------------------------
        
        print(f' [*] Model loaded successfully (RNG states ignored).')
...
def train(rank, problem, agent, val_dataset, tb_logger):
    ...
    for epoch in range(opts.epoch_start, opts.epoch_end):
        ...
        if opts.train_dataset is not None:
            training_dataset = problem.make_dataset(filename=opts.train_dataset, DUMMY_RATE=opts.dummy_rate, size=opts.graph_size, num_samples=opts.epoch_size)
        else:
...
def train_batch(
    ...
    obj, context = problem.get_costs(batch, solution, get_context=True, check_full_feasibility=False)
    obj = torch.cat((obj[:,None],obj[:,None],obj[:,None]),-1).clone()
    context2 = torch.zeros(bs,9).to(solution.device);context2[:,-1] = 1
    feasibility_history = torch.tensor(feasibility_history_base).view(-1,total_history).expand(bs, total_history).to(obj.device)
    action = None
